**v0.6.4** · Requires Node.js 18 or 20 and `@lxpack/cli` on your PATH.
...
See [lessonkit.json interchange](lessonkit-interchange.md) and [LessonKit interoperability](lessonkit-interoperability.md).

!!! tip "React / LessonKit authors"
    For [LessonKit 1.0](https://github.com/eddiethedean/lessonkit) projects, prefer **`lessonkit package --target scorm12`** (via `@lessonkit/lxpack`). Use `lxpack build --lessonkit` for interchange-only workflows without the LessonKit CLI.
