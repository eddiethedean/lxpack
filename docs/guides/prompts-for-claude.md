...
```text title="LXPack session context (v0.3.6)"
You are helping author an LXPack v0.3.6 course.
...
Write a Markdown lesson file for LXPack v0.3.6 only (no YAML, no HTML).
...
Write an LXPack v0.3.6 assessment YAML file (multiple choice only).
...
Write one index.html file for LXPack v0.3.6 (single file, inline CSS).
...
Design LXPack v0.3.6 variables: and flow: blocks for course.yaml.
...
You are migrating an existing multi-page HTML eLearning course into LXPack v0.3.6.
...
From the inventory below, produce an LXPack v0.3.6 file plan only (no full file contents yet).
...
Migrate this legacy HTML course into the LXPack course folder (v0.3.6). Work in the LXPack tree only; treat the HTML folder as read-only reference.
...
Convert this training outline into an LXPack v0.3.6 file plan (no file contents yet).
