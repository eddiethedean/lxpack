...
>
> **LessonKit [1.0.0](https://github.com/eddiethedean/lessonkit)** ships `@lessonkit/lxpack`, `@lessonkit/cli`, and the golden packaging example. **LXPack v0.4.0–v0.6.4** shipped SPA lessons, `@lxpack/api`, `packageLessonkit()`, `@lxpack/spa-bridge`, `@lxpack/conformance`, and `@lxpack/lessonkit`. See [LessonKit interoperability](lessonkit-interoperability.md) for current usage.
